...
export const CATEGORY_COLORS: Record<string, string> = {
  ...
  '단체샷': 'bg-indigo-100 text-indigo-700 border-indigo-200',
  ...
  '튜토리얼': 'bg-teal-100 text-teal-700 border-teal-200',
...
export const getCategoryColor = (cat: string): string => {
  return CATEGORY_COLORS[cat] || 'bg-slate-50 text-slate-500 border-slate-200';
};
